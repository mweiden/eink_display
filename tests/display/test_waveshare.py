...
from eink_display.display.waveshare import MockEPDDriver, WaveshareEPDDriver
...
class _FakeEPD:
    ...
    def display(self, buffer: bytes) -> None:
        self.display_calls.append(buffer)
...
class TestWaveshareEPDDriver:
    def test_initialize_and_display_flow(self) -> None:
        ...
        assert len(fake.display_calls) == 1
        assert fake.display_calls[0] == image.tobytes()
